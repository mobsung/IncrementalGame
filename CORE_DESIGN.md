...
- Superare un'ondata non resuscita le unità morte. La vita residua delle unità sopravvissute passa all'ondata successiva. La resurrezione può avvenire tramite abilità o con il Chrono break; dopo una sconfitta si ripristina lo stato registrato per il tentativo, che può ancora contenere unità morte.
...
Nella **prima demo**, l'unico requisito per evolvere è raggiungere una **soglia di livello configurabile per ciascuna evoluzione**. L'evoluzione viene attivata manualmente dal giocatore: raggiungere il livello richiesto la rende disponibile, ma non la esegue automaticamente. I valori delle soglie restano da definire. Per le versioni successive potranno essere introdotti requisiti aggiuntivi, ancora da progettare.

In futuro un **bestiario** mostrerà tutte le unità e le rispettive evoluzioni. Quando il giocatore decide di evolvere una copia, deve poter vedere e valutare dalla schermata di quella unità le opzioni disponibili, comprese le alternative ramificate. Numero di stadi, contenuti dei rami e vincolo o reversibilità della scelta restano da definire.
...
Il Chrono break resetta le ondate e riapre la possibilità di ottenere XP affrontandole nuovamente. **Non resetta** valuta globale, livelli, XP, stadio evolutivo, statistiche permanenti o potenziamenti delle unità; le unità e i loro investimenti rimangono. È sempre attivabile, anche durante un'ondata e con zero Chrono Shards ottenibili, senza attendere una pausa; riporta in vita e cura completamente le unità schierate. È il momento ordinario in cui si può cambiare quali unità occupano gli slot; fra Chrono break è consentito riposizionare quelle già in campo. I Chrono Shards base dipendono dalla **più alta ondata completata nella run**; iniziare un'ondata senza completarla non aumenta il premio. Se il Chrono break viene attivato durante un'ondata, il combattimento termina immediatamente: il Gold e l'XP generati in quell'ondata vengono scartati anche per i nemici già uccisi, mentre i nemici ancora vivi scompaiono senza concedere ricompense. Le ricompense delle ondate e dei tentativi conclusi in precedenza restano possedute. La curva iniziale dei Chrono Shards base è concordata; costi, utilizzi e possibili soglie sono ancora da sviluppare.
...
Quando inizierà l'implementazione, **tutti i valori di bilanciamento dovranno essere facilmente modificabili**: valori base, incrementi, coefficienti delle curve, costi iniziali, crescita dei costi, limiti, soglie e requisiti. Il principio vale per il Chrono shop e per tutte le altre meccaniche. La soluzione tecnica sarà definita nella fase di implementazione; per ora si continua con il design.

## Questioni ancora da sviluppare — punti 18–28

18. **Evoluzioni:** nella prima demo basta una soglia di livello configurabile per ciascuna evoluzione e l'attivazione è manuale. Una specie può non avere evoluzioni; in futuro potranno esserne aggiunte di lineari o ramificate. L'evoluzione modifica aspetto e abilità, non aumenta direttamente le statistiche base e restituisce i punti livello investiti; livello, XP e potenziamenti Gold rimangono. Restano da definire le soglie numeriche, il numero di stadi, i contenuti dei rami e la reversibilità della scelta. Il bestiario è previsto in futuro; le opzioni vanno valutate dalla schermata dell'unità.
...
22. **Regole residue delle ricompense:** contributo delle copie del giocatore schierate sul lato nemico ai bonus Gold e XP, eventuale loro accesso all'XP, visualizzazione o arrotondamento dei totali assegnati, punti anima delle uccisioni durante un'ondata interrotta dal Chrono break.
